...
#----------------------- SEGUNDA Y TERCER FORMA: Original (BHSig260) --------------------------
...
#-------- TERCERA Y CUARTA FORMA: Añadir dataset Dutch como dataset de train/test --------

def getImages4(loc,size,dic):
    print("Getting Dataset 4 Data..")
    real_names = os.listdir(os.path.join(loc,'genuine'))
    forg_names = os.listdir(os.path.join(loc,'forgery'))
    img_real = getData(os.path.join(loc,'genuine'),real_names,size,dic)
    img_forg = getData(os.path.join(loc,'forgery'),forg_names,size,dic)
    print("Data Import Complete!")
    return (np.asarray(img_real),np.asarray(img_forg))
